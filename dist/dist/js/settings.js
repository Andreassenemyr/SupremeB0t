const
	...
	editShipping = () => {
		Data.checkIfShippingFilled(r => {
			if (r.length == 0) {
				dsp("Informationen ar uppdaterad!", "success")
		...
		Array.prototype.forEach.call(document.getElementsByClassName("kwf"), (element, divIndex, divArray) => {
			...
			keywordfields.forEach((data, fieldIndex, fieldArray) => {
				...
				if (document.getElementById(fieldName).value == '') {
					error += "- Fältet " + fieldName + " är tomt.<br/>"
				} else {
	...
	newForm.innerHTML = '<table class="table table-sm" style="margin-top: 20px;">' +
                            '<tbody>' +
                                '<tr><td>Category</td><td>' +
                                    ...
                                    '</select>' +
                                '</td></tr>' +
                                '<tr><td>Nyckelord</td><td><input class="form-control" id="keyword['+formId+']" type="text"/></td></tr>' +
                                '<tr><td>Färg</td><td><input class="form-control" id="color['+formId+']" type="text"/></td></tr>' +
                                '<tr><td>Size</td><td>' +
...
const 
	...
	updateParams = () => {
		...
		dsp("Inställningarna har uppdaterats", "success")
	}
...
document.title = "SupremeBOT v" + chrome.runtime.getManifest().version
