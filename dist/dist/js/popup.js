	...
	startBot = () => {
		if (localStorage['data'].length > 4) {
			...
			if (Number.isInteger(startTime) && nowTime < startTime)
				...
				$("#loader").html("<u>Botten har startat</u>")
	...
	loaderCountdown = startTime => {
		...
		var refresh = setInterval(() => {
			...
			if (seconds == 0) {
				...
				$("#loader").html("<u>Botten startar</u>")
			} else
				$("#loader").html("Botten startar om " + seconds + " seconds.")
		}, 1000)
	}
...
document.getElementById("nbkws").innerHTML = (() => {
	...
	return '<b>Du har valt ' + nb + ' saker att choppa.</b>'
})()
